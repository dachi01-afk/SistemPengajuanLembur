            ...
            <div class="card-body">
                <div class="mb-2">
                    <button @click="isModalOpen = true"
                        class=" btnAdd inline-block px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">Tambah
                        Data <i class="fa-solid fa-user-plus"></i></button>
                </div>
                                        ...
                                        <td>
                                            <div class="flex items-center space-x-3">
                                                <button type="button" @click="isModalOpen = true"
                                                    class="text-purple-600 hover:text-purple-800 focus:outline-none editBtn"
                                                    data-id="{{ $item->id }}" data-department="{{ $item->department }}">
                                                    <i class="fa-solid fa-pen"></i>
                                                </button>
                                                <button type="button"
                                                    class="text-purple-600 hover:text-purple-800 focus:outline-none deleteBtn"
                                                    data-id="{{ $item->id }}">
            ...
            <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150"
                ...
                <form id="formDepartment" class="space-y-4">
                    ...
                    <label for="department" class="block text-sm">
                        ...
                        <input name="department" value="{{ old('department') }}" autocomplete="off"
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil',
                            text: isEdit ? 'Data berhasil diperbarui!' :
                                'Data berhasil disimpan!',
                        }).then(() => {
                            location.reload();
                            // Tutup modal
                            document.querySelector('[x-data]').__x.$data.isModalOpen =
                                false;
                            resetFormDepartment();
                        });
                    },
                    error: function(xhr) {
                        if (xhr.status === 422) {
                            ...
                            $.each(errors, function(key, messages) {
                                ...
                                input.after('<p class="mt-1 text-xs text-red-600">' +
                                    messages[0] + '</p>');
                            });
